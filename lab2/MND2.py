...
# Підбір значення для m
def find_m(m, table):
    key = list(table.keys())
    if m in key:
        return m
    else:
        differences = ([abs(m - i) for i in key])
...
def task(m):
    ...
    Rkr_table = {2: 1.73, 6: 2.16, 8: 2.43, 10: 2.62, 12: 2.75, 15: 2.9, 20: 3.08}
    variant = 9
    y_max = (30 - variant) * 10
    y_min = (20 - variant) * 10
    ...
    y = [[rand.randint(y_min, y_max) for j in range(6)] for i in range(m)]
    avg_Y = average_y(y)
    ...
    Rkr = Rkr_table[find_m(m, Rkr_table)]
    print(f'При m = {m} для p = 0.99 Rkr = {Rkr}')
